...
with open('data/TripChain/Truncated_theta_od_Minus.csv', 'rb') as theta_OD_dash:
...
cur.execute("truncate table demand.temp_trips_activities_shortterm_calibration_minus_simon")
cur.execute("truncate table demand.temp_subtrips_shortterm_calibration_minus_simon")
cur.execute("UPDATE demand.trips_activities_minus_simon SET load_factor=1")
cur.execute("UPDATE demand.subtrips_minus_simon SET load_factor=1")
...
if len(OD_counts) == len(OD_intervals):
    ...
    query = "select * from demand.trips_activities_minus_simon"
    ...
    query = "select * from demand.subtrips_minus_simon"
    ...
    tripsFileMinus = open('copy_trips_minus_simon.csv', 'w+')
    subTripsFileMinus = open('copy_subtrips_minus_simon.csv', 'w+')
    ...
    while row < totalRows:
		...
		print item
            ...
            while originalCount < count:
                ...
                tripsFileMinus.write('"%s","%s","%s","%s","%s",1,1,%s,%s,1\n' % (trip_id, person_id, startTime, empty, empty, origin, destination))
                subTripsFileMinus.write('"%s",1,"Car","%s",2,1,1,%s,%s,1\n' % (trip_id, empty, origin, destination))

                originalCount += 1

        row += 1

    tripsFileMinus.close()
    subTripsFileMinus.close()

    curr_path = os.getcwd()
    curr_path_trips = curr_path + '/copy_trips_minus_simon.csv'
    curr_path_sub_trips = curr_path + '/copy_subtrips_minus_simon.csv'

    #Execute the query
    query = "copy demand.temp_trips_activities_shortterm_calibration_minus_simon(trip_id, person_id, trip_start_time, activity_start_time, activity_end_time, "\
            " origin_type, destination_type, origin_id, destination_id,load_factor) from '" + curr_path_trips + "' csv"
    cur.execute(query)

    query = "copy demand.temp_subtrips_shortterm_calibration_minus_simon(trip_id, sequence_num, travel_mode, pt_line_id, cbd_traverse_type, origin_type, "\
    ...
    cur.execute("truncate table demand.subtrips_minus_simon")

    try:
        updated_st_file.seek(0)
        cur.copy_from(updated_st_file, 'demand.subtrips_minus_simon', sep=",")
    except:
        print "Error: Couldn't update demand.subtrips_minus table!!"
