...
with open('data/TripChain/Truncated_theta_od_Plus.csv', 'rb') as theta_OD_dash:
...
cur.execute("truncate table demand.temp_trips_activities_shortterm_calibration_plus_simon")
cur.execute("truncate table demand.temp_subtrips_shortterm_calibration_plus_simon")
cur.execute("UPDATE demand.trips_activities_plus_simon SET load_factor=1")
cur.execute("UPDATE demand.subtrips_plus_simon SET load_factor=1")
...
if len(OD_counts) == len(OD_intervals):
    ...
    query = "select * from demand.trips_activities_plus_simon"
    ...
    query = "select * from demand.subtrips_plus_simon"
    ...
    tripsFilePlus = open('copy_trips_plus_simon.csv', 'w+')
    subTripsFilePlus = open('copy_subtrips_plus_simon.csv', 'w+')
    ...
    tripsFilePlus.close()
    subTripsFilePlus.close()

    curr_path = os.getcwd()
    curr_path_trips = curr_path + '/copy_trips_plus_simon.csv'
    curr_path_sub_trips = curr_path + '/copy_subtrips_plus_simon.csv'

    #Execute the query
    query = "copy demand.temp_trips_activities_shortterm_calibration_plus_simon(trip_id, person_id, trip_start_time, activity_start_time, activity_end_time, "\
            " origin_type, destination_type, origin_id, destination_id,load_factor) from '" + curr_path_trips + "' csv"
    cur.execute(query)

    query = "copy demand.temp_subtrips_shortterm_calibration_plus_simon(trip_id, sequence_num, travel_mode, pt_line_id, cbd_traverse_type, origin_type, "\
    ...
    cur.execute("truncate table demand.subtrips_plus_simon")

    try:
        updated_st_file.seek(0)
        cur.copy_from(updated_st_file, 'demand.subtrips_plus_simon', sep=",")
    except:
        print "Error: Couldn't update demand.subtrips_plus table!!"
