...
sim_vehicle_counts <- read.table('SimMobility_Minus/avgVehicleCounts.csv', sep = ',')
...
# Mapping on lane level
 # Change lane id of ST to 
      #   OriginalSTlaneID <- as.data.frame(unique(sim_vehicle_counts$LaneID))
      #   CorrectedSTlaneID <- as.data.frame(unique(sim_vehicle_counts$LaneID))
      #   ReferenceTable_STlaneID <- cbind(OriginalSTlaneID, CorrectedSTlaneID); colnames(ReferenceTable_STlaneID) <- c('Original', 'Corrected')
      #   write.table(ReferenceTable_STlaneID, file=('ReferenceTable_STlaneID.csv')) 
  #ReferenceTable_STlaneID <- read.table('ReferenceTable_STlaneID.csv') 
...
# File export
  # data/truecount_Minus.csv
  write.table(Data_Seg$Count_Real, file='data/truecount_Minus.csv', row.names = F, col.names=F)
  # data/simcount_Minus.csv
  write.table(Data_Seg$Count_Sim, file='data/simcount_Minus.csv', row.names = F, col.names=F)
  # data/trueCountsRM_Minus.csv
  write.table(Data_Seg$Count_Real, file='data/trueCountsRM_Minus.csv', row.names = F, col.names=F)
  # data/simCountsRM_Minus.csv
  write.table(Data_Seg$Count_Sim, file='data/simCountsRM_Minus.csv', row.names = F, col.names=F)
  # data/compare_counts_Minus.csv
  write.table(Data_Seg, file='data/compare_counts_Minus.csv', row.names = F, col.names=F)
  # data/trueCountsVar_Minus.csv
  write.table(Data_Seg$Var_Real, file='data/trueCountsVar_Minus.csv', row.names = F, col.names=F)
  # data/trueCountsObs_Minus.csv
  write.table(Data_Seg$Obs_Real, file='data/trueCountsObs_Minus.csv', row.names = F, col.names=F)
