...
od_interval = int(sys.argv[3]) * 60
sensor_interval = int(sys.argv[4]) * 60

num_interval_od = int((end_time - start_time) / od_interval)
num_interval_sensor = int((end_time - start_time) / sensor_interval)

assignment_matrix = numpy.genfromtxt('SimMobility/assignment_matrix.csv', delimiter=',')
od_pair = numpy.genfromtxt('data/TripChain/ODpair.csv', delimiter=" ").astype(int)
ref_sensor = list(csv.reader(open("data/ref_segments.csv", "rb"), delimiter=" "))
...
for item in assignment_matrix:
	if ref_sensor_dict.__contains__(int(item[1])):
		sensor_index = ref_sensor_dict[int(item[1])]
...
with open('data/WMATRIX/wmatrix.csv', 'w') as fp:
